...
function newNote (req, res, next) {
  ...
  models.Note.create({
    ...
    // Sandstorm single note hack
    return res.redirect(config.serverURL + '/RrRSKqpiEeiY0FKSafsUWQ');
...
function showPublishNote (req, res, next) {
  ...
  findNote(req, res, function (note) {
    ...
    note.increment('viewcount').then(function (note) {
      ...
      fs.writeFileSync('/var/www/index.html', '<script>window.location.href = "/s/RrRSKqpiEeiY0FKSafsUWQ";</script>');
      // End of Sandstorm hack
