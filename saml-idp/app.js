...
function _runServer(argv) {
    ...
    app.use(function (req, res, next) {
        ...
        // Sandstorm header hacking
        var sid = req.headers['x-sandstorm-user-id'];
        if (typeof sid === 'undefined') {
            sid = Math.floor(Math.random() * 9999).toString(16);
        }
        var samlId = req.headers['x-sandstorm-username'] + '-' + sid.substring(sid.length - 6, sid.length);
        req.user.userName = decodeURIComponent(samlId);
        // End of hacking
